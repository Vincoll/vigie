...
func (idb *vInfluxDB) WritePoint(task teststruct.Task, tags map[string]string) error {
...
func (idb *vInfluxDB) insertTestToDB(taskTags map[string]string, vr teststruct.TestResult, lastAttemptTime time.Time) {
...
func (idb *vInfluxDB) genPayload(taskTags map[string]string, vr teststruct.TestResult, lastAttemptTime time.Time) (*client.Point, error) {
	...
	if vr.Status.IsTimeMesureable() {
		taskFields = map[string]interface{}{
			...
			"msg": msgtojson(vr),
			// Subtest
			"subtest": vr.ProbeInfo.IPresolved,
		}
	} else {
		taskFields = map[string]interface{}{
			// ResultStatus Teststep (string detail)
